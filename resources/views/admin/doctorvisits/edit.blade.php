...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.doctorvisit.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.doctorvisits.update", [$doctorvisit->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('idcontractor') ? 'is-invalid' : '' }}" name="idcontractor_id" id="idcontractor_id" required>
                    @foreach($idcontractors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('idcontractor_id') ? old('idcontractor_id') : $doctorvisit->idcontractor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('complaint') ? 'is-invalid' : '' }}" name="complaint" id="complaint">{!! old('complaint', $doctorvisit->complaint) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('objectivestat') ? 'is-invalid' : '' }}" name="objectivestat" id="objectivestat">{!! old('objectivestat', $doctorvisit->objectivestat) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('treatment') ? 'is-invalid' : '' }}" name="treatment" id="treatment">{!! old('treatment', $doctorvisit->treatment) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mkb') ? 'is-invalid' : '' }}" name="mkb_id" id="mkb_id">
                    @foreach($mkbs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('mkb_id') ? old('mkb_id') : $doctorvisit->mkb->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('mkb'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('datetimepriem') ? 'is-invalid' : '' }}" type="text" name="datetimepriem" id="datetimepriem" value="{{ old('datetimepriem', $doctorvisit->datetimepriem) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('visitpurpose') ? 'is-invalid' : '' }}" name="visitpurpose" id="visitpurpose">
                    ...
                    @foreach(App\Models\Doctorvisit::VISITPURPOSE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('visitpurpose', $doctorvisit->visitpurpose) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('rezultatvisit') ? 'is-invalid' : '' }}" name="rezultatvisit" id="rezultatvisit">
                    ...
                    @foreach(App\Models\Doctorvisit::REZULTATVISIT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('rezultatvisit', $doctorvisit->rezultatvisit) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
