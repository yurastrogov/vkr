    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.paramedics.update", [$paramedic->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_user') ? 'is-invalid' : '' }}" name="id_user_id" id="id_user_id" required>
                    @foreach($id_users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('id_user_id') ? old('id_user_id') : $paramedic->id_user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('surname') ? 'is-invalid' : '' }}" type="text" name="surname" id="surname" value="{{ old('surname', $paramedic->surname) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fathername') ? 'is-invalid' : '' }}" type="text" name="fathername" id="fathername" value="{{ old('fathername', $paramedic->fathername) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('speciality') ? 'is-invalid' : '' }}" name="speciality_id" id="speciality_id" required>
                    @foreach($specialities as $id => $entry)
                        <option value="{{ $id }}" {{ (old('speciality_id') ? old('speciality_id') : $paramedic->speciality->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shedule') ? 'is-invalid' : '' }}" name="shedule_id" id="shedule_id" required>
                    @foreach($shedules as $id => $entry)
                        <option value="{{ $id }}" {{ (old('shedule_id') ? old('shedule_id') : $paramedic->shedule->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
