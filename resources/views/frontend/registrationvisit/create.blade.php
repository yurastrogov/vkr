    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.registrationvisit.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="idcontractor_id" id="idcontractor_id" required>
                                @foreach($idcontractors as  $contractor)
                                    <option value="{{ $contractor->id }}" {{ old('idcontractor_id') == $contractor->id ? 'selected' : '' }}>{{ $contractor->surname }} {{ $contractor->name }} {{ $contractor->fathername }}
                                        {{ $contractor->berthday }} Снилс:{{ $contractor->snils }} Полис:{{ $contractor->polis }}   </option>
                                @endforeach
                            </select>
